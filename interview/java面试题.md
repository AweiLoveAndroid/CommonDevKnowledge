...
* 静态内部类的设计意图
>**答：** 静态内部类可以独立存在，又希望只被外部类使用，private修饰的时候不被同一个包下的其他类使用。
...
* 静态代理和动态代理的区别，什么场景使用？
>**答：** [看看定义说说优缺点，大概](http://blog.csdn.net/hejingyuan6/article/details/36203505)
...
* 说一下泛型原理，并举例说明
>**答：** [看看这里，里面还有点其他泛型相关的面试题](http://blog.csdn.net/sunxianghuang/article/details/51982979)
...
* 讲一下对图的理解
>**答：** 不理解不理解 [了解看这里](http://www.cnblogs.com/skywang12345/p/3691463.html)
...
* ReentrantLock的内部实现
>**答：** [看这里](http://blog.csdn.net/endlu/article/details/51249156)

* lock原理
>**答：** 同上

* 死锁的四个必要条件？
>**答：** 定义：线程死锁是指由于两个或者多个线程互相持有对方所需要的资源，导致这些线程处于等待状态，无法前往执行。当线程进入对象的synchronized代码块时，便占有了资源，直到它退出该代码块或者调用wait方法，才释放资源，在此期间，其他线程将不能进入该代码块。当线程互相持有对方所需要的资源时，会互相等待对方释放资源，如果线程都不主动释放所占有的资源，将产生死锁。  
...
* 对象锁和类锁是否会互相影响？
>**答：** 类锁和对象锁不是同1个东西，一个是类的Class对象的锁，一个是类的实例的锁。
>线程访问静态synchronized的时候，允许另一个线程访问对象的实例synchronized方法。反过来也是成立的，因为他们需要的锁是不同的。

* 什么是线程池，如何使用?
>**答：** 线程池是一种多线程处理形式，处理过程中将任务添加到队列，然后在创建线程后自动启动这些任务，线程池中线程的数目永远不会超过最大值，并且可以重复使用。
>线程池包括以下四个基本组成部分：  
1、线程池管理器（ThreadPool）：用于创建并管理线程池，包括 创建线程池，销毁线程池，添加新任务；
2、工作线程（PoolWorker）：线程池中线程，在没有任务时处于等待状态，可以循环的执行任务；
3、任务接口（Task）：每个任务必须实现的接口，以供工作线程调度任务的执行，它主要规定了任务的入口，任务执行完后的收尾工作，任务的执行状态等；
4、任务队列（taskQueue）：用于存放没有处理的任务。提供一种缓冲机制。
>常见线程池：newSingleThreadExecutor，newFixedThreadExecutor，newCacheThreadExecutor，newScheduleThreadExecutor
>[详细看这里](http://blog.csdn.net/wolf909867753/article/details/77500625)

* Java的并发、多线程、线程模型
>**答：** [这个问题问得有点广，点这](http://blog.csdn.net/zhangliangzi/article/details/52489720)
...
* 谈谈你对多线程同步机制的理解？
* 如何保证多线程读写文件的安全？
>**答：** 对文件的不同部分无交错地进行操作就不用加锁，不然还是加锁

* 多线程断点续传原理
>**答：** 
>1. 通过 HttpURLConnection.getContentLength() 获取文件长度
>2. 创建相同大小的本地文件，用temp后缀标记，表示未完成
>3. 尽量平均分每个线程下载的byte长度，分别计算每个线程下载起点
>4. 开启多线程下载，用urlConnection.setRequestProperty("Range","bytes=" + start + "-" + length)获取相应的字节
>5. 用RandomAccessFile.seek()方法开始分别写入下载的数据，并不断记录当前线程中已下载的字节数，方便断点后重新计算start和没有下载完的length
>6. 如果停止下载，则记录各个线程的下载情况到数据库。
>7. 重新开启下载，读取数据库数据，重复4，5。
>8. 直到下载完成，把文件重命名为正确名称，结束。
...
----
##### ps:答案可能存在错误，或者不准确，其实个人比较担心传播错误的知识，但是本着分析的精神还是提交上来了，如果有发现错误的地方希望大家能提交 pull request 或者在 issue 中指出，最好是一道一道提的修改，这样能减少单次核对的工作量。
